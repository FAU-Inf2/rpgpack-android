...
public class TableFragment extends GeneralFragment {
	...
	// also set in createTableHeader !! onCreate is called when orientation is
	// rotated ->
	// amountColumns = #before change but we recreate the header table with 2
	// columns
	int amountColumns = 2;
	AlertDialog dialog;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		LayoutInflater inflater = (LayoutInflater) TemplateGeneratorActivity.theActiveActivity
				.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		View dialogViewTableView = inflater.inflate(
				R.layout.alertdialog_template_generator_tableview, null);
		dialogTable = ((TableLayout) dialogViewTableView
				.findViewById(R.id.tableView_alert_table));
		dialogRowCounter = (SessionMonitorEditText) dialogViewTableView
				.findViewById(R.id.edit_spaltenanzahl);
		dialogRowCounter
				.setOnEditSessionCompleteListener(new OnEditSessionCompleteListener() {
					@Override
					public void onEditSessionComplete(TextView v) {
						int valueGiven = (Integer.parseInt(v.getText()
								.toString()));
						if (valueGiven < 0) {
		...
		ImageButton addButton = (ImageButton) dialogViewTableView
				.findViewById(R.id.button_add_column);
		addButton.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				int oldValue = (Integer.parseInt(dialogRowCounter.getText()
						.toString()));
				int newValue = oldValue + 1;
				dialogRowCounter.setText(Integer.toString(newValue));
				adaptDialogTable(dialogTable, (Integer
						.parseInt(dialogRowCounter.getText().toString())));
			}
		});
		ImageButton subtractButton = (ImageButton) dialogViewTableView
		...
		AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(
				TemplateGeneratorActivity.theActiveActivity);
		alertDialogBuilder.setView(dialogViewTableView);
		alertDialogBuilder
				.setCancelable(false)
				.setPositiveButton(R.string.save_table,
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int id) {
								setAmountOfColumns(Integer
										.parseInt(dialogRowCounter.getText()
												.toString()));
								setTypeContents();
			...
			for (int k = 0; k < table.getChildCount(); k++) {
				...
				if (choice.equals("Textfeld")) {
					...
					if (!(elementToAdapt instanceof CheckBox)) {
						isModified = true;
						// Log.d("TableFragment",
						// "setTypeContents: remove Elementt");
						tableRow.removeView(elementToAdapt);
					...
					if (!(elementToAdapt instanceof TextView)
							|| elementToAdapt instanceof EditText) {
						isModified = true;
						tableRow.removeView(elementToAdapt);
						newElement = new TextView(getActivity());
						((TextView) newElement).setText(R.string.new_element);
						LayoutInflater inflater = (LayoutInflater) TemplateGeneratorActivity.theActiveActivity
								.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
						final View popupView = inflater.inflate(
								R.layout.table_view_popup,
								(ViewGroup) getActivity().findViewById(
										R.id.popup_element));
						final PopupWindow popup = new PopupWindow(popupView,
								300, 370, true);
						popup.setOutsideTouchable(true);
						// AlertDialog.Builder alertCreater = new
						...
						// alertCreater.setPositiveButton("Ok", new
						...
						// });
						//
						// alertCreater.setNegativeButton("Cancel", new
						// DialogInterface.OnClickListener() {
						// public void onClick(DialogInterface dialog, int
						// whichButton) {
						// // Canceled.
						...
						newElement.setOnClickListener(new OnClickListener() {
							...
							public void onClick(View v) {
								// alert.show();
								popup.showAtLocation(popupView, Gravity.CENTER,
										0, 0);
							}
						});
						// setTableStyle(newElement);
				...
				if (isModified) {
					newElement.measure(MeasureSpec.UNSPECIFIED,
	...
	protected void adaptHeaderTable(TableLayout dialogTable) {
		...
		for (int i = 1; i < amountRows + 1; i++) {
			String otherString = ((EditText) ((TableRow) dialogTable
					.getChildAt(i)).getChildAt(1)).getText().toString();
			EditText textToChange = ((EditText) ((TableRow) headerTable
					.getChildAt(0)).getChildAt(i - 1));
	...
	protected TableLayout createTableHeader() {
		...
		col1.setText(R.string.headline1);
		...
		col2.setText(R.string.headline2);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mainView = (RelativeLayout) inflater.inflate(
				R.layout.template_generator_table_view, null);
		createTableHeader();
		table = (TableLayout) mainView
				.findViewById(R.id.template_generator_table);
		// set the 3 buttons
		...
		Button addColumnButton = (Button) mainView
				.findViewById(R.id.add_column);
		addColumnButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				addColumn();
			}
		});
		Button removeColumnButton = (Button) mainView
				.findViewById(R.id.remove_column);
		 ...
		 * JACKSON START
		 */
		Log.d("TableFragment", "jacksonInflateWithData:"
				+ jacksonInflateWithData);
		final int jacksonTableColumnNumber = jacksonTable.numberOfColumns;
		// check if we have inflated the table with some data
		// BUT also check if we got any saved columns (they are only created if
		// user goes into table!)
		// so if there are no saved columns or we didn't load any data we add
		// the default columns
		if (jacksonTableColumnNumber > 0) {
			// set flag, so we don't add new columns to the jackson table while
			// loading
			jacksonInflateWithData = true;
			// create the right amount of columns
			Log.d("jackson table inflating", "jacksonNumberCol:"
			...
			for (int i = 0; i < amountColumns; i++) {
				...
				Log.d("TABLE INFLATING", "setting column(" + i
						+ ") header title:"
						+ jacksonTable.columnHeaders.get(i).name);
				((EditText) view)
						.setText(jacksonTable.columnHeaders.get(i).name);
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		MenuInflater inflater = TemplateGeneratorActivity.theActiveActivity
				.getMenuInflater();
	...
	protected void addItemList() {
		...
		table.addView(row);
		final ScrollView sv = (ScrollView) mainView
				.findViewById(R.id.table_scroll);
		// note: we have to do scrolling in seperate thread to make sure the new
		...
		for (int i = 0; i < ((TableRow) headerTable.getChildAt(0))
				.getChildCount(); i++) {
			setHeaderTableStyle((EditText) ((TableRow) headerTable
	...
	/**
	 * checks if it is needed to resize the given row (does it if textEdited is
	 * bigger now)
	 * 
	 * @param width
	 * @param height
	 ...
	 * @param row
	 */
	protected void checkResize(int width, int height, View textEdited,
			TableRow row) {
		...
		if (usedTable.getChildAt(indexOfRow) instanceof TableRow) {
			indexOfColumn = ((TableRow) usedTable.getChildAt(indexOfRow))
					.indexOfChild(textEdited);
		}
		int largestWidth = 0;
		...
		/*
		 * Code that measures the height of one row int largestHeight = 0; if
		 * (row instanceof TableRow) { for(int i=0; i<((TableRow)
		 * row).getChildCount(); i++){ View view = ((TableRow)
		 * row).getChildAt(i); int localHeight = 0; if(view instanceof
		 * EditText){ view.measure(MeasureSpec.UNSPECIFIED,
		 * MeasureSpec.UNSPECIFIED); localHeight = view.getMeasuredHeight();
		 * if(localHeight > largestHeight){ largestHeight = localHeight; } } } }
		 */
	...
	protected void adaptAllColumnsToSize(int columnIndex, int width) {
		...
		if (singleRow instanceof TableRow) {
			View theChildToResize = ((TableRow) singleRow)
					.getChildAt(columnIndex);
			if (theChildToResize instanceof EditText) {
				// setHeaderTableStyle((EditText) theChildToResize);
				// TODO: extract measurement of height to match the hight of the
				// biggest element in the row
				theChildToResize.measure(MeasureSpec.UNSPECIFIED,
						MeasureSpec.UNSPECIFIED);
				int height = theChildToResize.getMeasuredHeight();
				final TableRow.LayoutParams lparams = new TableRow.LayoutParams(
		...
		for (int i = 0; i < table.getChildCount(); i++) {
			...
			if (oneRow instanceof TableRow) {
				View theChildToResize = ((TableRow) oneRow)
						.getChildAt(columnIndex);
				if (theChildToResize instanceof EditText) {
					// TODO: extract measurement of height to match the hight of
					// the biggest element in the row
					theChildToResize.measure(MeasureSpec.UNSPECIFIED,
							MeasureSpec.UNSPECIFIED);
					int height = theChildToResize.getMeasuredHeight();
					final TableRow.LayoutParams lparams = new TableRow.LayoutParams(
	...
	protected void addColumnToRow(final TableRow row) {
		...
		if (row == headerTable.getChildAt(0)) {
			// guard. only set column title to default if we are not currently
			// inflating with data
			/*
			 * if(!jacksonInflateWithData) {
			 * jacksonTable.setColumnTitle(((TableRow
			 * )headerTable.getChildAt(0)).getChildCount(), "Headline " +
			 * (((TableRow) headerTable.getChildAt(0)).getChildCount()+1)); }
			 */
			oneColumn.addTextChangedListener(new TextWatcher() {
				// column index for this header cell !index! no + 1 needed
				private final int columnIndex = ((TableRow) headerTable
						.getChildAt(0)).getChildCount();

				// callback
				public void afterTextChanged(Editable s) {
					// guard. only set column title if we are not currently
					// inflating with data
					if (!jacksonInflateWithData) {
						// only save if the title changed
						if (jacksonTable.setColumnTitle(columnIndex,
								s.toString())) {
							Log.d("TABLE_FRAGMENT", "column title changed");
							// TemplateGeneratorActivity.saveTemplateAsync();
						}
		...
		if (headerTable.indexOfChild(row) != -1) {
			...
			oneColumn
					.setText(R.string.headline
							+ " "
							+ (((TableRow) headerTable.getChildAt(0))
									.getChildCount() + 1));
		} else {
			setTableStyle(oneColumn);
			oneColumn.setText(R.string.blank);
		}
	...
	protected void removeColumn() {
		...
		if (amountColumns < 1) {
			 ...
			 * JACKSON START
			 */
			// guard. only remove column if we are not currently inflating with
			// data
			if (!jacksonInflateWithData) {
	...
	class DialogSpinnerAdapter<T> extends ArrayAdapter<T> {
		...
		public View getDropDownView(int position, View convertView,
			...
			text.setTextColor(getResources().getColor(R.color.background));// choose
																			// your
																			// color
																			// :)
			/* } */
	...
	/**
	 * adapt the table to match the amount and names of rows given in the
	 * mainView
	 * 
	 * @param dialogTable
	 */
	protected void adaptDialogTable(TableLayout dialogTable) {
		...
		// int oldColumnsToAdept = (firstRowToAdd<rowsNeeded?
		// firstRowToAdd:rowsNeeded);
		// for(int i=1; i<oldColumnsToAdept; i++){
		// TableRow headerRow = (TableRow) headerTable.getChildAt(0);
		// EditText headerText = (EditText) headerRow.getChildAt(i-1);
		// EditText textToSet = (EditText) ((TableRow)
		// dialogTable.getChildAt(i)).getChildAt(1);
		// textToSet.setText(headerText.getText());
		// }
		// //second step: add rows if needed
		// for(int i=firstRowToAdd-1; i<rowsNeeded; i++){
		// Log.d("dialog", "add");
		// final TableRow row = new
		// TableRow(TemplateGeneratorActivity.theActiveActivity);
		// TableRow.LayoutParams rowParams = new TableRow.LayoutParams();
		...
		// if(k==2 && i != -1){
		// Spinner spin = new
		// Spinner(TemplateGeneratorActivity.theActiveActivity);
		// String [] spin_arry =
		// getResources().getStringArray(R.array.spaltentypen);
		// spin.setAdapter(new
		// DialogSpinnerAdapter<CharSequence>(TemplateGeneratorActivity.theActiveActivity,
		// spin_arry));
		// theView = spin;
		...
		// arg2, long arg3) {
		// ((TextView)
		// parent.getChildAt(0)).setTextColor(getResources().getColor(R.color.background));
		// }
		// @Override
		...
		// }
		// else{
		// final EditText oneColumn = new
		// EditText(TemplateGeneratorActivity.theActiveActivity);
	...
	protected void adaptDialogTable(TableLayout dialogTable, int rowsToShow) {
		...
		// first step: adapt all needed Column-names from headerTable
		int oldColumnsToAdept = (firstRowToAdd < rowsNeeded ? firstRowToAdd
				: rowsNeeded);
		for (int i = 1; i < oldColumnsToAdept; i++) {
			TableRow headerRow = (TableRow) headerTable.getChildAt(0);
			EditText headerText = (EditText) headerRow.getChildAt(i - 1);
			EditText textToSet = (EditText) ((TableRow) dialogTable
			...
			for (int k = 0; k < 3; k++) {
				...
				if (k == 2 && i != -1) {
					...
					String[] spin_arry = getResources().getStringArray(
							R.array.spaltentypen);
					spin.setAdapter(new DialogSpinnerAdapter<CharSequence>(
							TemplateGeneratorActivity.theActiveActivity,
							spin_arry));
					theView = spin;
					setSpinnerStyle(spin);
					...
					final EditText oneColumn = new EditText(
							TemplateGeneratorActivity.theActiveActivity);
					setTableStyle((EditText) oneColumn);
					if (i == -1) {
						if (k == 0) {
							oneColumn.setText(R.string.number);
						} else if (k == 1) {
							oneColumn.setText(R.string.column_name);
						} else {
							oneColumn.setText(R.string.column_type);
						}
					}
					// insert the number of the column in the first row
					else if (k == 0) {
						oneColumn.setText((i + 1) + R.string.dot);
					} else if (k == 1) {
		...
		// last step: remove rows if needed
		for (int i = rowsNeeded + 1; i < firstRowToAdd; i++) {
			dialogTable.removeView(dialogTable.getChildAt(dialogTable
					.getChildCount() - 1));
		}
	}
	 ...
	 * setAmountOfColumns(params[0]); // adaptDialogTable(((TableLayout)
	 * dialogViewTableView.findViewById(R.id.tableView_alert_table))); return
	 * null; }
	 * 
